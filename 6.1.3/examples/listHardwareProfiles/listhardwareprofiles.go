...
func main() {
	...
	if hardwareprofiles, err := ctx.GetHardwareProfiles(); err != nil {
		fmt.Printf("Error when calling GetHardwareProfiles(): %s\n", err)
		os.Exit(1)
	} else {
		fmt.Println("Following hardware-profiles nodes found:")
		for _, profile := range hardwareprofiles.Hardwareprofiles {
			fmt.Printf("%s\n", profile.Name)
		}
	}
}
